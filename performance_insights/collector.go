...
func getEnabledInstances(instances []sessions.Instance) []sessions.Instance {
	...
	for _, instance := range instances {
		if instance.DisablePerformanceInsights {
			continue
		}
...
func (c *Collector) Update(sessions *sessions.Sessions, logger log.Logger) {
	level.Info(c.logger).Log("msg", "Canceling the old performance insights scraper")
	c.cancelFunc()
	level.Info(c.logger).Log("msg", "The old performance insights scraper has been canceled")
